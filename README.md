...
| 구분 | 기능 |
| --- | --- |
| 선수 관리 | 개별·일괄 등록, 이름 검색, 급수·성별·게임 수 관리, 휴식·복귀 상태 전환 |
| 코트 운영 | 코트 추가·삭제, 자율·남복·여복·혼복 설정, 진행 경기와 다음 경기 동시 관리 |
| 매칭 | 게임 수가 적은 선수 우선 자동 매칭, 4명 수동 매칭, 파트너 중복 최소화, 팀 밸런싱 |
| 현장 대응 | 배정 선수 교체, 다음 경기 예약 취소, 다른 코트로 예약 이동, 자동 승계 |
| 기록 | 경기 시작·종료 시각, 소요 시간, 코트, 참여 선수, 팀별 점수·승패·점수 차 기록 |
...
4. 코트마다 `자율`, `남복`, `여복`, `혼복` 중 경기 유형을 선택합니다.
...
7. 모임 종료 후 `⚙️ 설정 → 💾 데이터 백업`으로 JSON 파일을 보관합니다.
